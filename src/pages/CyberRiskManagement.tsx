...
} from "@/data/cyberRiskManagement";

const WorkflowPanel = ({ step }: { step: CrmWorkflowStep }) => (
  <motion.div
    key={step.step}
  ...
  >
    ...
    <div className="mt-8">
      <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-primary">Connected to</p>
...
const CyberRiskManagement = () => {
  const [activeStep, setActiveStep] = useState(0);
  const [activePill, setActivePill] = useState(0);
  const currentWorkflow = crmWorkflow[activeStep];

  useEffect(() => {
    const interval = setInterval(() => {
      setActivePill((current) => (current + 1) % crmHero.heroPills.length);
    }, 2800);
  ...
  return (
    <div className="min-h-screen bg-background bg-gradient-main">
          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <SlideInLeft>
              <div>
                ...
                <h1 className="text-4xl font-display font-bold md:text-5xl lg:text-6xl">{crmHero.headline}</h1>
                <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{crmHero.subheadline}</p>
                ...
                <div className="mt-8 min-h-[52px]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {(() => {
                        const pill = crmHero.heroPills[activePill];
                        return (
                          <>
            ...
            <SlideInRight>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                {crmHero.outcomes.map((outcome, index) => (
                  <motion.div
                    key={outcome.label}
      ...
      <section className="border-y border-border/30 bg-card/20 py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">{crmProblem.title}</h2>
          </FadeUp>

          <StaggerContainer className="mt-12 grid gap-6 md:grid-cols-2" staggerDelay={0.08}>
            {crmProblem.painPoints.map((point) => (
        ...
        <div className="container mx-auto px-6">
          <div className="mb-14 max-w-3xl">
            <FadeUp>
              ...
              <h2 className="text-3xl font-display font-bold md:text-5xl">{crmSmartWay.title}</h2>
            </FadeUp>
          </div>

          <div className="grid gap-6 lg:grid-cols-2">
            {crmSmartWay.items.map((item, index) => (
              <SlideInLeft key={item.title} delay={index * 0.05}>
                <motion.div
        ...
        <div className="container relative z-10 mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">
              How Observeri cyber risk management works
            </h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
              As controls and checks run, Observeri keeps risk scores and heatmaps current in the background—so teams
              see what changed, what needs attention now, and where to focus next.
            </p>
          </FadeUp>

          <div className="mt-14 grid gap-8 lg:grid-cols-[0.4fr_0.6fr]">
            <div className="space-y-3">
              {crmWorkflow.map((step, index) => (
                <motion.button
                  key={step.step}
                  type="button"
      ...
      <section className="py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">
              Everything behind smarter risk management
            </h2>
          </FadeUp>
      ...
      <section className="relative overflow-hidden py-24">
        ...
        <div className="container relative z-10 mx-auto px-6">
          <FadeUp>
            <p className="mb-4 text-sm font-medium uppercase tracking-[0.25em] text-primary">Outcomes</p>
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">
              What happens when risk stays live
            </h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
              Continuous visibility, faster action, and proof that stays current.
            </p>
          </FadeUp>

          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {crmLiveOutcomes.map((outcome, index) => (
            ...
            <ScaleUp>
              <div className="rounded-[2rem] border-gradient bg-gradient-card p-10">
                ...
                <h2 className="mt-4 text-3xl font-display font-bold md:text-4xl">
                  The Observeri advantage for cyber risk
                </h2>
                <p className="mt-4 text-sm leading-7 text-muted-foreground">
                  Observeri turns risk management into a live, connected system. Every risk stays tied to assets,
                  controls, and owners—scores update as systems change, and everyone works from the same clear picture
                  of exposure and progress.
                </p>
            ...
            <div className="relative overflow-hidden rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              ...
              <div className="relative z-10 mx-auto max-w-3xl text-center">
                <h2 className="text-3xl font-display font-bold md:text-5xl">
                  Turn live risk intelligence into action
                </h2>
                <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">
                  Connect your risk register to real controls, assets, and AI prioritization—so your team always knows
                  what changed, what matters, and what is verified.
                </p>
                <div className="mt-9 flex flex-col justify-center gap-4 sm:flex-row">
                  ...
                  <Button asChild variant="outline" size="lg">
                    <Link to="/products/third-party-risk">Explore Third Party Risk</Link>
                  </Button>
                </div>
              </div>
            </div>
...
export default CyberRiskManagement;
