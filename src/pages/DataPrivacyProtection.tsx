...
import {
  ...
  dppValueProp,
  dppWorkflow,
  type DppWorkflowStep,
} from "@/data/dataPrivacyProtection";

const WorkflowPanel = ({ step }: { step: DppWorkflowStep }) => (
  <motion.div
    key={step.step}
...
const DataPrivacyProtection = () => {
  ...
  const currentWorkflow = dppWorkflow[activeStep];

  useEffect(() => {
    const interval = setInterval(() => {
      setActivePill((current) => (current + 1) % dppHero.heroPills.length);
    }, 2800);
  ...
  return (
    <div className="min-h-screen bg-background bg-gradient-main">
      ...
      <section className="relative overflow-hidden pb-20 pt-32">
        ...
        <motion.div
          className="absolute right-1/4 top-1/3 h-96 w-96 rounded-full bg-primary/10 blur-3xl"
          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <SlideInLeft>
              <div>
                ...
                <h1 className="text-4xl font-display font-bold md:text-5xl lg:text-6xl">{dppHero.headline}</h1>
                <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{dppHero.subheadline}</p>
                ...
                <div className="mt-8 min-h-[52px]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {(() => {
                        const pill = dppHero.heroPills[activePill];
                        return (
                          <>
                ...
                <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                  ...
                  <Button asChild variant="outline" size="lg">
                    <a href="#ropa">See ROPA & data flows</a>
                  </Button>
                </div>
              </div>
            </SlideInLeft>
            ...
            <SlideInRight>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                {dppHero.outcomes.map((outcome, index) => (
                  <motion.div
                    key={outcome.label}
      ...
      {/* ROPA showcase */}
      <section id="ropa" className="border-y border-border/30 bg-card/20 py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            <p className="mb-4 text-sm font-medium uppercase tracking-[0.25em] text-primary">
              Register of Processing Activities
            </p>
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">{dppRopaShowcase.title}</h2>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">{dppRopaShowcase.description}</p>
          </FadeUp>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {dppRopaShowcase.stats.map((stat, index) => (
          ...
          <FadeUp delay={0.15}>
            <div className="mt-10 flex flex-wrap items-center gap-3">
              <span className="rounded-full border border-border/40 bg-background/30 px-4 py-2 text-sm text-muted-foreground">
                {dppRopaShowcase.activityExample.id} — {dppRopaShowcase.activityExample.name}
              </span>
              <span className="rounded-full border border-primary/30 bg-primary/10 px-4 py-2 text-sm text-primary">
                {dppRopaShowcase.activityExample.legalBasis}
              </span>
              <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-sm text-orange-400">
                Risk: {dppRopaShowcase.activityExample.riskLevel}
              </span>
            </div>
          </FadeUp>

          <StaggerContainer className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4" staggerDelay={0.07}>
            {dppRopaShowcase.flowStages.map((stage, index) => (
              <StaggerItem key={stage.title}>
                <div className="relative h-full">
                  <motion.div
                    className={`h-full rounded-2xl border p-5 ${stage.accent}`}
                    whileHover={{ y: -4 }}
                  ...
                  {index < dppRopaShowcase.flowStages.length - 1 ? (
                    <ChevronRight className="absolute -right-3 top-1/2 hidden h-4 w-4 -translate-y-1/2 text-primary/50 xl:block" />
                  ) : null}
                </div>
              </StaggerItem>
            ))}
          ...
          <FadeUp delay={0.2}>
            <div className="mt-8 rounded-2xl border border-dashed border-sky-500/40 bg-sky-500/5 p-6">
              <div className="flex items-start gap-4">
                <div className="rounded-xl bg-sky-500/10 p-3">
                  <Globe className="h-5 w-5 text-sky-400" />
                </div>
                <div>
                  <p className="font-display font-semibold">{dppRopaShowcase.crossBorder.title}</p>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Countries: {dppRopaShowcase.crossBorder.countries}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    Safeguard: {dppRopaShowcase.crossBorder.safeguard}
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">{dppProblem.title}</h2>
          </FadeUp>

          <StaggerContainer className="mt-12 grid gap-6 md:grid-cols-2" staggerDelay={0.08}>
            {dppProblem.painPoints.map((point) => (
      ...
      <section id="sub-modules" className="border-y border-border/30 bg-card/20 py-24">
        <div className="container mx-auto px-6">
          <ScaleUp>
            <div className="rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              <div className="mx-auto max-w-4xl text-center">
                <p className="mb-4 text-sm font-medium uppercase tracking-[0.25em] text-primary">
                  {dppValueProp.eyebrow}
                </p>
                <h2 className="text-3xl font-display font-bold md:text-5xl">{dppValueProp.title}</h2>
                <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
                  {dppValueProp.description}
                </p>
              </div>

              <StaggerContainer className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" staggerDelay={0.06}>
                {dppSubModules.map((subModule) => (
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-3">
                {dppValueProp.pillars.map((pillar, index) => (
                  <motion.div
                    key={pillar.label}
        ...
        <div className="container relative z-10 mx-auto px-6">
          <FadeUp>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
              A continuous privacy intelligence loop—from source connection through NER-powered discovery to
              exposure-scored remediation.
            </p>
          </FadeUp>

          <div className="mt-14 grid gap-8 lg:grid-cols-[0.4fr_0.6fr]">
            <div className="space-y-3">
              {dppWorkflow.map((step, index) => (
                <motion.button
                  key={step.step}
                  type="button"
              ...
              <motion.div
                ...
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
              >
                <p className="text-xs uppercase tracking-[0.16em] text-primary">Privacy loop</p>
      ...
      <section className="relative overflow-hidden py-24">
        ...
        <div className="container relative z-10 mx-auto px-6">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {dppOrgOutcomes.map((outcome, index) => (
            ...
            <ScaleUp>
              <div className="rounded-[2rem] border-gradient bg-gradient-card p-10">
                ...
                <ul className="mt-8 space-y-4">
                  {dppCapabilities.map((capability, index) => (
                    <motion.li
                      key={capability}
            ...
            <div className="space-y-6">
              ...
              <FadeUp delay={0.1}>
                <div className="rounded-[2rem] border border-primary/20 bg-primary/10 p-8">
                  <p className="text-sm uppercase tracking-[0.2em] text-primary">Regulatory alignment</p>
                  <p className="mt-4 text-sm leading-7 text-muted-foreground">
                    Data Privacy & Protection supports GDPR, UAE PDPL, HIPAA, and regional data protection frameworks—
                    connecting discovery and classification evidence to Compliance Management and Exposure Management
                    for end-to-end privacy programme governance.
                  </p>
                </div>
              </FadeUp>
            </div>
            ...
            <div className="relative overflow-hidden rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              ...
              <div className="relative z-10 mx-auto max-w-3xl text-center">
                ...
                <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">
                  PII detection, NER-powered identification, secret scanning, and data exposure scoring across every
                  connected source—built for privacy teams that need proof, not promises.
                </p>
