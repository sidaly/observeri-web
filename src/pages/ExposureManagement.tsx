...
import {
  ...
  expValueProp,
  expWorkflow,
  type ExpWorkflowStep,
} from "@/data/exposureManagement";

const WorkflowPanel = ({ step }: { step: ExpWorkflowStep }) => (
  <motion.div
    key={step.step}
...
const ExposureManagement = () => {
  ...
  const currentWorkflow = expWorkflow[activeStep];

  useEffect(() => {
    const interval = setInterval(() => {
      setActivePill((current) => (current + 1) % expHero.heroPills.length);
    }, 2800);
          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <SlideInLeft>
              <div>
                ...
                <h1 className="text-4xl font-display font-bold md:text-5xl lg:text-6xl">{expHero.headline}</h1>
                <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{expHero.subheadline}</p>
                ...
                <div className="mt-8 min-h-[52px]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {(() => {
                        const pill = expHero.heroPills[activePill];
                        return (
                          <>
            ...
            <SlideInRight>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                {expHero.outcomes.map((outcome, index) => (
                  <motion.div
                    key={outcome.label}
      ...
      <section className="border-y border-border/30 bg-card/20 py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">{expProblem.title}</h2>
          </FadeUp>

          <StaggerContainer className="mt-12 grid gap-6 md:grid-cols-2" staggerDelay={0.08}>
            {expProblem.painPoints.map((point) => (
          ...
          <ScaleUp>
            <div className="rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              <div className="mx-auto max-w-4xl text-center">
                <p className="mb-4 text-sm font-medium uppercase tracking-[0.25em] text-primary">
                  {expValueProp.eyebrow}
                </p>
                <h2 className="text-3xl font-display font-bold md:text-5xl">{expValueProp.title}</h2>
                <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
                  {expValueProp.description}
                </p>
              </div>

              <StaggerContainer className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5" staggerDelay={0.06}>
                {expSubModules.map((subModule) => (
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-3">
                {expValueProp.pillars.map((pillar, index) => (
                  <motion.div
                    key={pillar.label}
        ...
        <div className="container relative z-10 mx-auto px-6">
          <FadeUp>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
              A continuous exposure reduction loop—from automated assessment planning through centralized remediation
              and live attack surface and threat monitoring.
            </p>
          </FadeUp>

          <div className="mt-14 grid gap-8 lg:grid-cols-[0.4fr_0.6fr]">
            <div className="space-y-3">
              {expWorkflow.map((step, index) => (
                <motion.button
                  key={step.step}
                  type="button"
              ...
              <motion.div
                ...
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
              >
                <p className="text-xs uppercase tracking-[0.16em] text-primary">Exposure loop</p>
      ...
      <section className="relative overflow-hidden py-24">
        ...
        <div className="container relative z-10 mx-auto px-6">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {expOrgOutcomes.map((outcome, index) => (
            ...
            <ScaleUp>
              <div className="rounded-[2rem] border-gradient bg-gradient-card p-10">
                ...
                <ul className="mt-8 space-y-4">
                  {expCapabilities.map((capability, index) => (
                    <motion.li
                      key={capability}
              ...
              <FadeUp delay={0.1}>
                <div className="rounded-[2rem] border border-primary/20 bg-primary/10 p-8">
                  ...
                  <p className="mt-4 text-sm leading-7 text-muted-foreground">
                    Exposure Management integrates with the AI Risk Operations Center, Vulnerability Operations, and
                    Compliance Management—so every remediated issue updates asset risk scores and reduces enterprise
            ...
            <div className="relative overflow-hidden rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              ...
              <div className="relative z-10 mx-auto max-w-3xl text-center">
                ...
                <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">
                  Plan tests, track issues with SLAs, remediate in a dedicated hub, monitor your attack surface, and
                  stay ahead of emerging threats—all in one exposure programme.
                </p>
