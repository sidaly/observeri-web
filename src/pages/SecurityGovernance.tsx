...
import {
  ...
  govValueProp,
  govWorkflow,
  type GovWorkflowStep,
} from "@/data/securityGovernance";

const WorkflowPanel = ({ step }: { step: GovWorkflowStep }) => (
  <motion.div
    key={step.step}
...
const SecurityGovernance = () => {
  ...
  const currentWorkflow = govWorkflow[activeStep];

  useEffect(() => {
    const interval = setInterval(() => {
      setActivePill((current) => (current + 1) % govHero.heroPills.length);
    }, 2800);
  ...
  return (
    <div className="min-h-screen bg-background bg-gradient-main">
      ...
      <section className="relative overflow-hidden pb-20 pt-32">
        ...
        <motion.div
          className="absolute right-1/3 top-24 h-80 w-80 rounded-full bg-primary/10 blur-3xl"
          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <SlideInLeft>
              <div>
                ...
                <h1 className="text-4xl font-display font-bold md:text-5xl lg:text-6xl">{govHero.headline}</h1>
                <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{govHero.subheadline}</p>
                ...
                <div className="mt-8 min-h-[52px]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {(() => {
                        const pill = govHero.heroPills[activePill];
                        return (
                          <>
                ...
                <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                  ...
                  <Button asChild variant="outline" size="lg">
                    <a href="#sub-modules">Explore sub-modules</a>
                  </Button>
                </div>
              </div>
            </SlideInLeft>
            ...
            <SlideInRight>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                {govHero.outcomes.map((outcome, index) => (
                  <motion.div
                    key={outcome.label}
      ...
      <section className="border-y border-border/30 bg-card/20 py-24">
        <div className="container mx-auto px-6">
          <FadeUp>
            ...
            <h2 className="max-w-3xl text-3xl font-display font-bold md:text-5xl">{govProblem.title}</h2>
          </FadeUp>

          <StaggerContainer className="mt-12 grid gap-6 md:grid-cols-2" staggerDelay={0.08}>
            {govProblem.painPoints.map((point) => (
          ...
          <ScaleUp>
            <div className="rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              <div className="mx-auto max-w-4xl text-center">
                <p className="mb-4 text-sm font-medium uppercase tracking-[0.25em] text-primary">
                  {govValueProp.eyebrow}
                </p>
                <h2 className="text-3xl font-display font-bold md:text-5xl">{govValueProp.title}</h2>
                <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
                  {govValueProp.description}
                </p>
              </div>

              <StaggerContainer className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5" staggerDelay={0.06}>
                {govSubModules.map((subModule) => (
              ...
              <div className="mt-10 grid gap-6 md:grid-cols-3">
                {govValueProp.pillars.map((pillar, index) => (
                  <motion.div
                    key={pillar.label}
        ...
        <div className="container relative z-10 mx-auto px-6">
          <FadeUp>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-muted-foreground">
              A continuous governance cycle connecting metrics, policies, budgets, controlled change, and project
              delivery—so security leadership steers with live data.
            </p>
          </FadeUp>

          <div className="mt-14 grid gap-8 lg:grid-cols-[0.4fr_0.6fr]">
            <div className="space-y-3">
              {govWorkflow.map((step, index) => (
                <motion.button
                  key={step.step}
                  type="button"
              ...
              <motion.div
                ...
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
              >
                <p className="text-xs uppercase tracking-[0.16em] text-primary">Governance cycle</p>
      ...
      <section className="relative overflow-hidden py-24">
        ...
        <div className="container relative z-10 mx-auto px-6">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {govOrgOutcomes.map((outcome, index) => (
            ...
            <ScaleUp>
              <div className="rounded-[2rem] border-gradient bg-gradient-card p-10">
                ...
                <ul className="mt-8 space-y-4">
                  {govCapabilities.map((capability, index) => (
                    <motion.li
                      key={capability}
              ...
              <FadeUp delay={0.1}>
                <div className="rounded-[2rem] border border-primary/20 bg-primary/10 p-8">
                  ...
                  <p className="mt-4 text-sm leading-7 text-muted-foreground">
                    Security Governance feeds from and informs the AI Risk Operations Center, Compliance Management, and
                    Executive Dashboards—so KRI/KPI metrics, budget decisions, and project outcomes reflect live risk
                    and compliance posture across the platform.
                  </p>
                </div>
              </FadeUp>
            </div>
            ...
            <div className="relative overflow-hidden rounded-[2rem] border-gradient bg-gradient-card p-10 md:p-14">
              ...
              <div className="relative z-10 mx-auto max-w-3xl text-center">
                ...
                <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">
                  Unify KRI/KPI monitoring, policy management, AI budgeting, change management, and project delivery in
                  one governance workspace built for CISOs and boards.
                </p>
